...
class StreamManager:
            ...
            while not stop_event.is_set():
                ...
                if fire_status in ["fire", "smoke"]:
                    ...
                    if (current_time - last_fire_notification_time) >= self.fire_cooldown_duration:
                        ...
                        # if user_email:
                        #     email_sent = await self.send_fire_alert_email(
                        #         user_email, camera_name, fire_status, location_info
                        #     )
                        #     if email_sent:
                        #         logger.info(f"Fire alert email sent to {user_email} for stream {stream_id_str}")
                        #     else:
                        #         logger.warning(f"Failed to send fire alert email to {user_email} for stream {stream_id_str}")
                        # else:
                        #     logger.warning(f"No email found for user {owner_id}, skipping email notification")
                        
                        # Update the last notification time
                        self.fire_notification_cooldowns[stream_id_str] = current_time
                        
                        logging.info(f"Fire/smoke notification (app + email) sent for stream {stream_id_str}: {fire_status}")
    ...
    def generate_fire_alert_message1(self, fire_status: str, stream_id_str: str) -> str:
        """Generate fire alert message with cooldown information."""
        base_message = f"Fire/smoke detected: {fire_status}"
    ...
    #                     # Thread-safe access to cooldown tracking
    #                     last_fire_notification_time = self.fire_notification_cooldowns.get(stream_id_str, 0)
                        
    #                     # Calculate time since last notification
    #                     time_since_last_notification = current_time - last_fire_notification_time
    #                     cooldown_period = self.fire_cooldown_duration  # 600 seconds = 10 minutes

    #                     # Debug logging
    #                     logger.debug(f"Fire detected in {stream_id_str}: status={fire_status}, "
    ...
    #                             alert_message = self.generate_fire_alert_message(fire_status, stream_id_str)
    #                             await self.add_notification(
    #                                 str(owner_id), str(workspace_id), stream_id_str, 
    #                                 camera_name, "alert", alert_message
    #                             )
    ...
    #                             #     )
    #                             #     if email_sent:
    #                             #         logging.info(f"Fire alert email sent to {user_email} for stream {stream_id_str}")
    #                             #     else:
    #                             #         logging.warning(f"Failed to send fire alert email to {user_email} for stream {stream_id_str}")
    #                             # else:
    #                             #     logging.warning(f"No email found for user {owner_id}, skipping email notification")
                                
    #                             # Update the last notification time AFTER successful notification
    #                             self.fire_notification_cooldowns[stream_id_str] = current_time
                                
    #                             logging.info(f"Fire/smoke notification sent for stream {stream_id_str}: {fire_status}. "
    #                                     f"Next notification allowed in {cooldown_period/60:.1f} minutes.")
                                        
    #                         except Exception as e:
    #                             logging.error(f"Error sending fire/smoke notification for stream {stream_id_str}: {e}", exc_info=True)
    #                             # Don't update cooldown time if notification failed
    #                     else:
    #                         # Calculate remaining cooldown time
    #                         remaining_cooldown = cooldown_period - time_since_last_notification
    #                         remaining_minutes = remaining_cooldown / 60.0
                            
    #                         # Log at different levels based on how much time has passed
    #                         if remaining_minutes > 5:
    #                             # Only log every minute for longer cooldowns to avoid spam
    #                             if int(remaining_minutes) % 2 == 0:  # Log on minute boundaries
    #                                 logging.debug(f"Fire/smoke detected in stream {stream_id_str} but still in cooldown. "
    #                                             f"Remaining: {remaining_minutes:.1f} minutes")
    #                         else:
    #                             # Log more frequently for last 5 minutes
    #                             logging.debug(f"Fire/smoke detected in stream {stream_id_str} but still in cooldown. "
    #                                         f"Remaining: {remaining_cooldown:.1f} seconds")
    ...
    #         # Cleanup cooldown tracking when stream ends
    #         self.fire_notification_cooldowns.pop(stream_id_str, None)
    #         logging.info(f"Cleaned up fire notification cooldown for stream {stream_id_str}")
    ...
    async def detect_objects_with_threshold1(self, frame: np.ndarray, conf_threshold: float = 0.4, 
                                                        threshold_settings: Dict[str, Any] = None) -> tuple[np.ndarray, int, bool, int, int, str]:
    ...
    def detect_objects_with_threshold_parallel1(self, frame: np.ndarray, conf_threshold: float = 0.4, 
                                                        threshold_settings: Dict[str, Any] = None) -> tuple[np.ndarray, int, bool, int, int, str]:
    ...
    async def send_fire_alert_email(self, user_email: str, camera_name: str, fire_status: str, 
    ...
    async def get_fire_notification_status(self, stream_id_str: str) -> Dict[str, Any]:
        """Get comprehensive fire notification status including email capability"""
        current_time = time.time()
        last_notification_time = self.fire_notification_cooldowns.get(stream_id_str, 0)
        
        # Get stream info to check if email is available
        try:
            stream_info = await self.db_manager.execute_query(
                """SELECT vs.user_id, u.email, vs.name 
                FROM video_stream vs 
                JOIN users u ON vs.user_id = u.user_id 
                WHERE vs.stream_id = $1""",
                (UUID(stream_id_str),), fetch_one=True
            )
            has_email = bool(stream_info and stream_info['email'])
            user_email = stream_info['email'] if stream_info else None
        except Exception as e:
            logger.error(f"Error checking email availability for stream {stream_id_str}: {e}")
            has_email = False
            user_email = None
        ...
        if last_notification_time == 0:
            return {
                ...
                "user_email": user_email if has_email else "Not available"
            }
