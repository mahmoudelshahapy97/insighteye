...
class ONNXModelLoader(BaseModelLoader):
    ...
    def postprocess(self, outputs: np.ndarray, original_shape: Tuple[int, int],
        ...
        """
        Postprocess YOLO v8 outputs
        
        YOLO v8 output format: (1, 84, 8400) where:
        - First 4 values: bounding box coordinates (x_center, y_center, width, height)
        - Remaining 80 values: class probabilities (for COCO dataset)
        
        Args:
            outputs: Model outputs (1, 84, 8400) or (1, 8400, 84)
        ...
        """
        # Handle different output formats - YOLO v8 typically outputs (1, 84, 8400)
        if len(outputs.shape) == 3:
            # If shape is (1, 8400, 84), transpose to (1, 84, 8400)
            if outputs.shape[2] > outputs.shape[1]:
                outputs = outputs.transpose(0, 2, 1)
            
            # Now shape should be (1, 84, 8400)
        ...
        # YOLO v8 format: [x_center, y_center, width, height, class_0_prob, class_1_prob, ...]
        boxes = predictions[:, :4]  # (8400, 4) - x_center, y_center, width, height
        class_scores = predictions[:, 4:]  # (8400, 80) - class probabilities
        
        # Get class with highest score and the score value
