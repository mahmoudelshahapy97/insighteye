...
async def test_fire_alert(
    stream_id: str,
    fire_status: str = "fire",
    current_user: dict = Depends(session_manager.get_current_user_full_data_dependency)
):
    """
    TEST ENDPOINT: Send a test fire alert to check if popup works.
    """
    try:
        stream_uuid = UUID(stream_id)
        ...
        # Build location
        location_parts = []
        if stream_info.get('building'):
            location_parts.append(stream_info['building'])
        if stream_info.get('floor_level'):
            location_parts.append(f"Floor {stream_info['floor_level']}")
        if stream_info.get('zone'):
            location_parts.append(stream_info['zone'])
        ...
        notification = await notification_service.create_notification(
            workspace_id=stream_info['workspace_id'],
            user_id=UUID(current_user['user_id']),  # Send to current user for testing
            status="urgent",
            message=message,
            stream_id=stream_uuid,
            camera_name=stream_info['name']
        ...
        return {
            "status": "success",
            "message": f"Test fire alert sent to {sent_count} connection(s)",
            "notification": notification_data,
            "hint": "Check your NotificationMenu - popup should appear with fire/smoke alert"
        }
