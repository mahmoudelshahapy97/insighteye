...
class StreamManager:
    ...
    async def start_stream_background(
        ...
        async with self._safe_stream_operation(stream_id_str, "start"):
            ...
            try:
                ...
                logger.info(
                    f"🔒 ATTEMPTING LOCK CLAIM: stream={stream_id_str}, server={server_id}"
                )
                ...
                if current_state:
                    logger.info(
                        f"📊 Current DB state: "
                        f"locked_by={current_state['locked_by_server']}, "
                        f"is_streaming={current_state['is_streaming']}, "
                ...
                if not verify_result:
                    logger.error(f"❌ VERIFICATION FAILED: Camera not found in DB")
                    raise RuntimeError("Camera disappeared from database")
                
                claimed_server = str(verify_result['locked_by_server']) if verify_result['locked_by_server'] else None
                
                if claimed_server != str(server_id):
                    logger.error(
                        f"❌ LOCK VERIFICATION FAILED: "
                        f"Expected server={server_id}, "
                        f"but got locked_by_server={claimed_server}"
                    )
                    raise RuntimeError(
                        f"Lock claim failed - locked by {claimed_server} instead of {server_id}"
                    )
                
                if not verify_result['is_streaming']:
                    logger.error(
                        f"❌ STREAMING STATE WRONG: is_streaming={verify_result['is_streaming']}"
                    )
                    raise RuntimeError("is_streaming was not set to TRUE")
                
                logger.info(
                    f"✅ LOCK VERIFIED: stream={stream_id_str}, "
                    f"server={server_id}, "
                    f"heartbeat={verify_result['server_heartbeat']}"
                )
                ...
                logger.error(
                    f"❌ STREAM START FAILED: stream={stream_id_str}, error={e}", 
                    exc_info=True
                )
                ...
                try:
                    ...
                    logger.info(f"✅ Lock released: {stream_id_str}")
                except Exception as release_err:
                    logger.error(
                        f"❌ Failed to release lock: {release_err}", 
                        exc_info=True
                    )
    ...
    async def stop_stream_in_workspace(
        ...
        """
        🛑 ATOMIC stop with zero race conditions.
        
        CRITICAL ORDERING:
        1. Verify permissions
        2. Signal stop in memory FIRST (prevents new frames)
        3. Release distributed lock SECOND (unblocks other servers)
        4. Update database THIRD (records final state)
        5. Clean up memory FOURTH
        
        This order ensures:
        - No gap where distributed manager can claim
        - Database reflects actual state
        - Memory is clean before lock release
        """
        # Validate access
        stream_info, membership_info = await self.validate_workspace_stream_access(
            user_id=requester_user_id,
        ...
        # ========== STEP 1: Signal Stop in Memory ==========
        # This immediately stops frame processing
        async with self._lock:
            if stream_id_str in self.active_streams:
                stop_event = self.active_streams[stream_id_str].get('stop_event')
                if stop_event:
                    stop_event.set()
                    logger.info(f"✅ Stop event set for {stream_id_str}")
        ...
        # This MUST happen before database update to prevent race conditions
        if stop_reason == 'user_action':
            try:
                from app.services.distributed_stream_manager import distributed_stream_manager
                
                # Release lock with force_stop=True
                await distributed_stream_manager.release_camera_lock(
                    stream_id_str, 
                    reason="user_action",
                    force_stop=True  # This will also set is_streaming=FALSE in DB
                )
                
                logger.info(f"✅ Distributed lock released for {stream_id_str}")
                
            except ImportError:
                logger.warning("⚠️ Distributed manager not available")
            except Exception as e:
                logger.error(f"Error releasing distributed lock: {e}")
        
        # ========== STEP 3: Update Database ==========
        # ONLY if lock release didn't already do it
        if stop_reason == 'user_action':
            # Verify database state (lock release should have updated it)
            verify_query = """
                SELECT is_streaming, locked_by_server, auto_retry_enabled
                FROM video_stream
                WHERE stream_id = $1
            """
            ...
            if verify_result and verify_result['is_streaming']:
                # Fallback: Lock release failed, update directly
                logger.warning(f"⚠️ Lock release didn't update DB, forcing update")
            ...
            try:
                ...
                logger.warning(f"Failed to cancel retry: {retry_err}")
            
        else:
            # System error: Keep is_streaming=TRUE for retry
            update_query = """
                UPDATE video_stream 
                SET 
                    status = 'error',
        ...
        if stop_reason == 'user_action':
            ...
            if final_check['is_streaming']:
                logger.error(
                    f"❌ FINAL CHECK FAILED: {stream_id_str} still has is_streaming=TRUE!"
                )
                # Force final update
                await self.db_manager.execute_query(
                    """UPDATE video_stream 
        ...
        if stop_reason == 'user_action':
            ...
            return {
                ...
                "message": f"Camera stopped due to {stop_reason} - will auto-retry"
            }
    ...
    async def _stop_stream(self, stream_id_str: str, for_restart: bool = False):
        """
        🛑 Internal stop - does NOT release distributed lock.
        
        IMPORTANT: Distributed lock is released by:
        1. stop_stream_in_workspace() for user stops
        2. distributed_stream_manager for system errors
        
        This method ONLY cleans up memory and local state.
        """
        logger.info(f"🛑 Internal stop: {stream_id_str} (for_restart={for_restart})")
            ...
            try:
                ...
                if slots_available > 0:
                    claim_query = """
                        ...
                        UPDATE video_stream
                        SET locked_by_server = $2, server_heartbeat = NOW()
                        FROM available_cameras
                        WHERE video_stream.stream_id = available_cameras.stream_id
                        AND video_stream.is_streaming = TRUE  -- Re-verify
                        RETURNING *
                    """
                    # claim_query = """
                    #     WITH available_cameras AS (
    ...
    async def _check_stream_health(self):
        ...
        async with self._health_lock:
                ...
                try:
                    ...
                    if db_stream_state.get('stop_reason') == 'user_action':
                        logger.warning(
                            ...
                            f"Cleaning up from memory (user manually stopped)."
                        )
                        health_issues.append(f"{stream_id_str}: user stopped")
                        await self._stop_stream(stream_id_str, for_restart=False)
                        continue
                    ...
                    if is_temporary_failure:
                        ...
                        if stopped_at:
                            ...
                            if failure_age < grace_period:
                                logger.info(
                                    f"⏳ Camera {stream_id_str} in temporary failure state "
                    ...
                    # ===== Check memory state for frozen streams =====
                    async with self._lock:
                    ...
                    if last_activity_time_mem:
                        if last_activity_time_mem.tzinfo is None:
                            last_activity_time_mem = last_activity_time_mem.replace(
            ...
            if streams_to_restart_ids:
                logger.warning(
                    f"⚠️ Health check found {len(streams_to_restart_ids)} streams to restart"
                )
                for stream_id_to_restart_str in streams_to_restart_ids:
                    logger.info(f"🔄 Restarting frozen stream: {stream_id_to_restart_str}")
                    
                    # Mark as system error to trigger retry
                    try:
                        await self.video_stream_service.record_camera_stop(
                            stream_id=UUID(stream_id_to_restart_str),
                            stop_reason='system_error',
                            stopped_by=None,
                            additional_context='Health check detected frozen stream'
                        )
                    except Exception as record_err:
                        logger.error(f"Error recording stop: {record_err}")
                    
                    # Stop in memory (will be restarted by retry system)
                    await self._stop_stream(stream_id_to_restart_str, for_restart=True)
            
            # Log health summary
            if health_issues:
                logger.warning(
                    f"Health check issues:\n" + 
                    "\n".join(f"  - {issue}" for issue in health_issues)
                )
            else:
                logger.debug(
                    f"✅ Health check passed for {len(active_stream_ids_copy)} streams"
                )
