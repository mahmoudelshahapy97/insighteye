...
class SessionManager:
    ...
    async def blacklist_token(self, token: str, user_id: Union[str, UUID], expires_at_iso: Union[str, datetime], reason: Optional[str] = None) -> str:
        ...
        try:
            ...
            if expires_at_dt.tzinfo is None: # Ensure timezone-aware (UTC)
                expires_at_dt = expires_at_dt.replace(tzinfo=ZoneInfo("Africa/Cairo"))
        ...
        try:
            ...
            for db_token_pair in active_db_tokens or []: # Handle case where active_db_tokens might be None
                ...
                if access_token_data:
                    await self.blacklist_token(db_token_pair['access_token'], user_id_uuid, datetime.fromtimestamp(access_token_data.exp, tz=ZoneInfo("Africa/Cairo")), "force_token_rotation")
                
                refresh_token_data = await self.verify_token(db_token_pair['refresh_token'])
                if refresh_token_data:
                     await self.blacklist_token(db_token_pair['refresh_token'], user_id_uuid, datetime.fromtimestamp(refresh_token_data.exp, tz=ZoneInfo("Africa/Cairo")), "force_token_rotation")
    ...
    async def revoke_token(self, token: str, reason: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not token_data: # Token is invalid, expired, or already blacklisted (verify_token checks blacklist)
                ...
                try:
                    ...
                    if payload and payload.get("exp") and payload.get("user_id"):
                        ...
                        exp_dt = datetime.fromtimestamp(exp_val, tz=ZoneInfo("Africa/Cairo"))
            ...
            expires_at_dt_for_blacklist = datetime.fromtimestamp(token_data.exp, tz=ZoneInfo("Africa/Cairo"))
    ...
    async def invalidate_all_user_tokens(self, user_id_str: str): 
        ...
        try:
            ...
            for token_info in active_tokens:
                ...
                if access_token_data:
                    await self.blacklist_token(token_info['access_token'], user_id_uuid, datetime.fromtimestamp(access_token_data.exp, tz=ZoneInfo("Africa/Cairo")), "user_security_action_all")
                
                # Blacklist refresh token
                refresh_token_data = await self.verify_token(token_info['refresh_token'])
                if refresh_token_data:
                    await self.blacklist_token(token_info['refresh_token'], user_id_uuid, datetime.fromtimestamp(refresh_token_data.exp, tz=ZoneInfo("Africa/Cairo")), "user_security_action_all_paired")
