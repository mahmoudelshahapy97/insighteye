...
# 🔧 FIXED VERSION - Proper lock release with is_streaming coordination
...
class DistributedStreamManager:
    """
    FIXED: Distributed stream manager with proper is_streaming coordination.
    
    CRITICAL FIXES:
    1. release_camera_lock() now has conditional is_streaming update
    2. Distinguishes between "user stop" (set FALSE) and "restart" (keep TRUE)
    3. Always updates is_streaming when releasing for normal stops
    """
    ...
    async def claim_available_cameras(self, slots_available: int) -> List[Dict[str, Any]]:
        """
        Claim available cameras with strict validation.
        """
        ...
        claim_query = """
            ...
            -- ✅ CRITICAL: Re-verify is_streaming in UPDATE
            ...
            for camera in claimed_cameras:
                ...
                if not camera.get('is_streaming', True):
                    ...
                    await self.release_camera_lock(
                        stream_id, 
                        reason="claimed_but_not_streaming",
                        force_stop=True  # ✅ NEW: Force is_streaming=FALSE
                    )
                    continue
                ...
                if camera.get('stop_reason') in ('user_action', 'user_stop', 'manual_stop', 'admin_stop'):
                    ...
                    await self.release_camera_lock(
                        stream_id, 
                        reason="claimed_but_user_stopped",
                        force_stop=True  # ✅ NEW: Force is_streaming=FALSE
                    )
                    continue
                ...
                if not camera.get('auto_retry_enabled', True):
                    ...
                    await self.release_camera_lock(
                        stream_id, 
                        reason="claimed_but_retry_disabled",
                        force_stop=True  # ✅ NEW: Force is_streaming=FALSE
                    )
                    continue
    ...
    async def release_camera_lock(
        ...
        """
        ✅ FIXED: Release server lock with proper is_streaming coordination.
        
        Args:
            stream_id_str: Camera stream ID
            reason: Why lock is being released
            force_stop: If True, also set is_streaming=FALSE (for errors/user stops)
        
        CRITICAL LOGIC:
        - force_stop=True: Release lock AND set is_streaming=FALSE (permanent stop)
        - force_stop=False: Release lock but keep is_streaming=TRUE (for restart/takeover)
        """
        try:
            ...
            should_stop_streaming = force_stop or reason in [
                ...
                'manual_stop', 
                'admin_stop',
                'claimed_but_not_streaming',
                'claimed_but_user_stopped',
                'claimed_but_retry_disabled',
                'zombie_cleanup',
                'verification_failed'
            ]
            
            if should_stop_streaming:
                # ✅ CRITICAL FIX: Also set is_streaming=FALSE for permanent stops
                release_query = """
                    UPDATE video_stream
                    ...
                    WHERE stream_id = $1
                      AND locked_by_server = $2
                    RETURNING is_streaming, locked_by_server
                """
                ...
                if result:
                    logger.info(
                        f"🔓 Released lock AND stopped streaming for {stream_id_str} "
                ...
                # ✅ Keep is_streaming=TRUE for restart/takeover scenarios
                release_query = """
                    UPDATE video_stream
                    SET locked_by_server = NULL,
                        server_heartbeat = NULL,
                ...
                if result:
                    logger.info(
                        ...
                        f"(reason: {reason}, keeping is_streaming={result['is_streaming']})"
                    )
                else:
                    logger.warning(
                        f"⚠️ Failed to release lock for {stream_id_str}"
            ...
            logger.error(f"Error releasing camera lock for {stream_id_str}: {e}")

    async def start_camera_locally(self, camera_data: Dict[str, Any]):
        """Start camera with pre-start validation."""
        ...
        try:
            # Safety check
            verify_query = """
                SELECT stop_reason, auto_retry_enabled, last_activity, path, is_streaming
                FROM video_stream 
                WHERE stream_id = $1
            """
            ...
            if verify:
                # ✅ CRITICAL: Verify is_streaming is still TRUE
                if not verify.get('is_streaming', True):
                    logger.warning(
                        f"⚠️ Camera {stream_id_str} is_streaming changed to FALSE after claim, "
                        f"releasing lock and skipping start"
                    )
                    await self.release_camera_lock(
                        stream_id_str, 
                        reason="is_streaming_false_before_start",
                        force_stop=True
                    )
                    return
            ...
            # Release lock on failure with force_stop
            await self.release_camera_lock(
                stream_id_str, 
                reason="start_failed",
                force_stop=True  # ✅ Stop streaming on start failure
            )
    ...
    async def stop_camera_locally(self, stream_id_str: str, reason: str = "normal_stop"):
        ...
        try:
            ...
            # Release server lock with proper is_streaming coordination
            force_stop = reason in [
                'user_action',
                'management_loop_stop',
                'verification_failed',
                'database_mismatch'
            ]
        ...
        for zombie_id in zombies:
            try:
                await self.release_camera_lock(
                    ...
                    force_stop=True  # ✅ Zombies should stop streaming
                )
    ...
    async def manage_streams_with_deduplication(self):
        """
        Management loop with proper state coordination.
        """
            ...
            try:
                ...
                if local_stream_ids:
                    ...
                    for db_state in db_states:
                        ...
                        # ✅ CRITICAL: Stop if is_streaming=FALSE or user stopped
                        should_stop = (
    ...
    async def stop_management_loop(self):
        ...
        # Release all locks with force_stop
        async with self._lock:
        ...
        for stream_id_str in stream_ids_to_release:
            await self.release_camera_lock(
                ...
                force_stop=True  # ✅ Shutdown should stop streaming
            )
