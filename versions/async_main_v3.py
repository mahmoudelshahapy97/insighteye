...
# Environment variable to disable auto-initialization (for emergency use)
...
async def emergency_safe_startup():
    ...
    try:
        await async_init_db_pool()
        logger.info("Database connection pool initialized in safe mode")
        
        # Just check what exists without changing anything
        diagnosis = await diagnose_database_state()
        logger.info(f"Database diagnosis (read-only): {diagnosis.get('total_records', 0)} total records found")
