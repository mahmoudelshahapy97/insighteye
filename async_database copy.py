    ...
    async def execute_query(self, query: str, params: Optional[tuple] = None,
        ...
        async def _execute(conn_to_use: asyncpg.Connection):
            try:
                ...
                elif isinstance(db_err, asyncpg.exceptions.RaiseError):
                    # Handle custom database triggers (like your alert threshold validation)
                    error_message = str(db_err).strip()
                    if "count_threshold_less must be less than count_threshold_greater" in error_message:
                        raise HTTPException(
                            status_code=status.HTTP_400_BAD_REQUEST, 
                            detail="Invalid alert thresholds: count_threshold_less must be less than count_threshold_greater"
                        )
                    else:
                        raise HTTPException(
                            status_code=status.HTTP_400_BAD_REQUEST, 
                            detail=f"Database validation error: {error_message}"
                        )
                elif isinstance(db_err, asyncpg.exceptions.CheckViolationError):
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST, 
                        detail=f"Database constraint check failed: {db_err.detail or db_err.message}"
                    )
                else:
...
async def ensure_database_indices():
    ...
    try:
        ...
        if errors:
            # This is not necessarily a fatal error for startup, but should be logged prominently.
            logger.warning(f"Some non-critical errors occurred during index creation: {errors}")
            # Depending on policy, you might not want to raise HTTPException here if indices are "IF NOT EXISTS"
            # raise asyncpg.PostgresError(f"Partial failure in index creation: {errors}") # Original
    except asyncpg.PostgresError as e:
